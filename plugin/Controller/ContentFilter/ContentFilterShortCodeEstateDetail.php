...
class ContentFilterShortCodeEstateDetail
{
	...
	public function renderHtmlHelperUserIfEmptyEstateId(){
        ...
        $html = '<div>';
        $html.= '<div>' . esc_html_e('You have opened the detail page, but we do not know which estate to show you, because there is no estate ID in the URL. Please go to an estate list and open an estate from there.',
                'onoffice-for-wp-websites') . '</div>';
        if (is_user_logged_in()) {
            $html.= '<div>' . esc_html_e('Since you are logged in, here is a link to a random estate so that you can preview the detail page:',
                    'onoffice-for-wp-websites') . '</div>';
            $html.= '<a href=' . $ramdomEstateLink . '>' . esc_html(__('Beautiful home with great view',
    ...
    private function getRandomEstateDetail(){
