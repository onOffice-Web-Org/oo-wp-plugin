	...
	extends AdminPageSettingsBase
{
	...
	protected function updateValues(array $row, stdClass $pResult, $recordId = null)
	{
		...
		if ($recordId != 0) {
			...
			if (array_key_exists(RecordManager::TABLENAME_RECIPIENTS, $row)) {
				$result = $result && $pRecordManagerUpdateForm->updateRecipientsByRow($row[RecordManager::TABLENAME_RECIPIENTS]);
			}
		} else {
			...
			try {
				...
				$row[RecordManager::TABLENAME_RECIPIENTS] =
					$this->prepareRelationValues(RecordManager::TABLENAME_RECIPIENTS, 'form_id', $row, $recordId);
